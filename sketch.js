...
function drawPlaying() {
  ...
  if (questionBoxScale > 0.01) {
    ...
    if (quizSolved) {
      ...
      // [修改] 第三關題目較長，字體再縮小一點以容納更多文字
      if (currentLevel === 3) {
        qSize = max(14, qSize - 2);
      }
      ...
      // 題目文字打字機效果
      // [修改] 處理換行邏輯：強制每10個字換行 (包含標點符號)
      let processedQ = currentQuestion.q;
      // 使用正則表達式每10個字插入換行，(?!$) 避免在行尾插入多餘換行
      processedQ = processedQ.replace(/(.{10})(?!$)/g, '$1\n');
